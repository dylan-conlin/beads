TASK: Add 'could-not-reproduce' as distinct close outcome for bugs. Add --outcome flag to bd close or integrate with orch complete flow. Track in issue metadata, surface in bd stats. See issue orch-go-jjd4.3 for full spec.
...
- [orch-go] Workers Stall During Build Phase
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2025-12-22-inv-workers-stall-during-build-phase.md
...
1. Report via `bd comment orch-go-jjd4.3 "Phase: Planning - [brief description]"`
...
1. Run: `bd comment orch-go-jjd4.3 "Phase: Complete - [1-2 sentence summary of deliverables]"`
...
1. Surface it first: `bd comment orch-go-jjd4.3 "CONSTRAINT: [what constraint] - [why considering workaround]"`
...
2. **SET UP investigation file:** Run `kb create investigation add-not-reproduce-distinct-close` to create from template
   - This creates: `.kb/investigations/simple/YYYY-MM-DD-add-not-reproduce-distinct-close.md`
   - This file is your coordination artifact (replaces WORKSPACE.md)
   - If command fails, report to orchestrator immediately

   - **IMPORTANT:** After running `kb create`, report the actual path via:
     `bd comment orch-go-jjd4.3 "investigation_path: /path/to/file.md"`
...
You were spawned from beads issue: **orch-go-jjd4.3**

**Use `bd comment` for progress updates instead of workspace-only tracking:**

```bash
# Report progress at phase transitions
bd comment orch-go-jjd4.3 "Phase: Planning - Analyzing codebase structure"
bd comment orch-go-jjd4.3 "Phase: Implementing - Adding authentication middleware"
bd comment orch-go-jjd4.3 "Phase: Complete - All tests passing, ready for review"

# Report blockers immediately
bd comment orch-go-jjd4.3 "BLOCKED: Need clarification on API contract"

# Report questions
bd comment orch-go-jjd4.3 "QUESTION: Should we use JWT or session-based auth?"
```
...
**Why beads comments:** Creates permanent, searchable progress history linked to the issue. Orchestrator can track progress across sessions via `bd show orch-go-jjd4.3`.
...
1. `bd comment orch-go-jjd4.3 "Phase: Complete - [1-2 sentence summary]"`
2. `/exit`
