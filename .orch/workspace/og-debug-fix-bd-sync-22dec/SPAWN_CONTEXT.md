TASK: Fix bd sync prefix parsing bug - compound prefixes like orch-go- are incorrectly parsed as orch-. The prefix parser is likely splitting on first hyphen instead of matching the configured prefix. Error: 'prefix mismatch detected: database uses orch-go- but found issues with prefixes: [kb-cli- (17 issues) orch- (34 issues)]' when all 34 issues are actually orch-go-*. Check prefix detection logic in sync command.

🚨 CRITICAL - FIRST 3 ACTIONS:
You MUST do these within your first 3 tool calls:
1. Report via `bd comment bd-t1u4 "Phase: Planning - [brief description]"`
...
2. Run: `bd comment bd-t1u4 "Phase: Complete - [1-2 sentence summary of deliverables]"`
...
2. **SET UP investigation file:** Run `kb create investigation fix-bd-sync-prefix-parsing` to create from template
   - This creates: `.kb/investigations/simple/YYYY-MM-DD-fix-bd-sync-prefix-parsing.md`
   - This file is your coordination artifact (replaces WORKSPACE.md)
   - If command fails, report to orchestrator immediately
   - **IMPORTANT:** After running `kb create`, report the actual path via:
     `bd comment bd-t1u4 "investigation_path: /path/to/file.md"`
...
6. **CREATE SYNTHESIS.md:** Before completing, create `SYNTHESIS.md` in your workspace: /Users/dylanconlin/Documents/personal/beads/.orch/workspace/og-debug-fix-bd-sync-22dec/SYNTHESIS.md
...
You were spawned from beads issue: **bd-t1u4**

**Use `bd comment` for progress updates instead of workspace-only tracking:**

```bash
# Report progress at phase transitions
bd comment bd-t1u4 "Phase: Planning - Analyzing codebase structure"
bd comment bd-t1u4 "Phase: Implementing - Adding authentication middleware"
bd comment bd-t1u4 "Phase: Complete - All tests passing, ready for review"

# Report blockers immediately
bd comment bd-t1u4 "BLOCKED: Need clarification on API contract"

# Report questions
bd comment bd-t1u4 "QUESTION: Should we use JWT or session-based auth?"
```
...
**Why beads comments:** Creates permanent, searchable progress history linked to the issue. Orchestrator can track progress across sessions via `bd show bd-t1u4`.
...
1. Ensure SYNTHESIS.md is created and committed in your workspace.
2. `bd comment bd-t1u4 "Phase: Complete - [1-2 sentence summary]"`
3. `/exit`
