TASK: Implement epic readiness gate: bd create --type epic should require --understanding flag (or --no-understanding --reason). Also bd ready should warn on epics missing Understanding section. Pattern: Follow bug repro validation (--repro or --no-repro --reason). See orch-go investigation 2026-01-07-inv-epic-readiness-gate-understanding-section.md for full design. Files: cmd/bd/create.go (add validation), cmd/bd/ready.go (add warning).
...
1. Report via `bd comment orch-go-p3rkp "Phase: Planning - [brief description]"`
...
2. Run: `bd comment orch-go-p3rkp "Phase: Complete - [1-2 sentence summary of deliverables]"`
...
1. Surface it first: `bd comment orch-go-p3rkp "CONSTRAINT: [what constraint] - [why considering workaround]"`
...
2. **SET UP investigation file:** Run `kb create investigation implement-epic-readiness-gate-bd` to create from template
   - This creates: `.kb/investigations/simple/YYYY-MM-DD-implement-epic-readiness-gate-bd.md`
   - This file is your coordination artifact (replaces WORKSPACE.md)
   - If command fails, report to orchestrator immediately

   - **IMPORTANT:** After running `kb create`, report the actual path via:
     `bd comment orch-go-p3rkp "investigation_path: /path/to/file.md"`
...
6. **CREATE SYNTHESIS.md:** Before completing, create `SYNTHESIS.md` in your workspace: /Users/dylanconlin/Documents/personal/beads/.orch/workspace/be-feat-implement-epic-readiness-07jan-5f97/SYNTHESIS.md
...
You were spawned from beads issue: **orch-go-p3rkp**

**Use `bd comment` for progress updates instead of workspace-only tracking:**

```bash
# Report progress at phase transitions
bd comment orch-go-p3rkp "Phase: Planning - Analyzing codebase structure"
bd comment orch-go-p3rkp "Phase: Implementing - Adding authentication middleware"
bd comment orch-go-p3rkp "Phase: Complete - All tests passing, ready for review"

# Report blockers immediately
bd comment orch-go-p3rkp "BLOCKED: Need clarification on API contract"

# Report questions
bd comment orch-go-p3rkp "QUESTION: Should we use JWT or session-based auth?"
```
...
**Why beads comments:** Creates permanent, searchable progress history linked to the issue. Orchestrator can track progress across sessions via `bd show orch-go-p3rkp`.
...
1. Ensure SYNTHESIS.md is created and committed in your workspace.
2. `bd comment orch-go-p3rkp "Phase: Complete - [1-2 sentence summary]"`
3. `/exit`
