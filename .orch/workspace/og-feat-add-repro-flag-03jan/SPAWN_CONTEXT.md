TASK: Add --repro flag to bd create for bug type. When --type bug, require --repro 'steps/evidence' or --no-repro --reason 'why'. Store repro in issue schema, display in bd show. See orch-go issue orch-go-jjd4.1 for full spec.
...
- Headless Spawn Model Format Bug
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2025-12-23-debug-headless-spawn-model-format.md
...
- Token Limit Explosion Headless Spawn
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2025-12-23-inv-token-limit-explosion-headless-spawn.md
- Verify Spawn Works
...
1. Report via `bd comment orch-go-jjd4.1 "Phase: Planning - [brief description]"`
...
1. Run: `bd comment orch-go-jjd4.1 "Phase: Complete - [1-2 sentence summary of deliverables]"`
...
1. Surface it first: `bd comment orch-go-jjd4.1 "CONSTRAINT: [what constraint] - [why considering workaround]"`
...
2. **SET UP investigation file:** Run `kb create investigation add-repro-flag-bd-create` to create from template
   - This creates: `.kb/investigations/simple/YYYY-MM-DD-add-repro-flag-bd-create.md`
   - This file is your coordination artifact (replaces WORKSPACE.md)
   - If command fails, report to orchestrator immediately

   - **IMPORTANT:** After running `kb create`, report the actual path via:
     `bd comment orch-go-jjd4.1 "investigation_path: /path/to/file.md"`
...
You were spawned from beads issue: **orch-go-jjd4.1**

**Use `bd comment` for progress updates instead of workspace-only tracking:**

```bash
# Report progress at phase transitions
bd comment orch-go-jjd4.1 "Phase: Planning - Analyzing codebase structure"
bd comment orch-go-jjd4.1 "Phase: Implementing - Adding authentication middleware"
bd comment orch-go-jjd4.1 "Phase: Complete - All tests passing, ready for review"

# Report blockers immediately
bd comment orch-go-jjd4.1 "BLOCKED: Need clarification on API contract"

# Report questions
bd comment orch-go-jjd4.1 "QUESTION: Should we use JWT or session-based auth?"
```
...
**Why beads comments:** Creates permanent, searchable progress history linked to the issue. Orchestrator can track progress across sessions via `bd show orch-go-jjd4.1`.
...
1. `bd comment orch-go-jjd4.1 "Phase: Complete - [1-2 sentence summary]"`
2. `/exit`
