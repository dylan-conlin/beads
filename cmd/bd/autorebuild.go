// Package main provides auto-rebuild functionality for the bd CLI.
...
func shouldAutoRebuild() bool {
	...
	if os.Getenv("BD_NO_AUTOREBUILD") == "1" || os.Getenv("BD_NO_AUTOREBUILD") == "true" {
		return false
	}

	return shouldAutoRebuildCheck(SourceDir, Commit)
}
...
func shouldAutoRebuildCheck(srcDir, embeddedHash string) bool {
	...
	// Skip if commit hash not embedded (dev build)
